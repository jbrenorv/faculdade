...
void init()
{
    ...
    for(a = 0; a < MAX; a++)
    {
        // nome vazio indica posicao desocupada
        *student[a].name = '\0';
    }
}
    ...
    while (aux)
    {
        ...
        if(a != -1)
        {
            ...
            printf("\n\033[1m|%9s%10s%11s%9s%6.2f%35s\033[0m\n", 
                   ...
                   "|",
                   "IRA = ",
                   ((student[a].soma_mf_x_credito)/student[a].soma_creditos),
                   "|");
            ...
            {
                printf(
                    "|%12s%3s%3d%3s%6.2f%3s%6.2f%3s%6.2f%3s%6s%3s%6.2f%3s%22s%3s\n",
...
void new_discipline(int a, int op)
{
    ...
    for (c = aux; c < (aux +num); c++)
    {
        ...
        if (student[a].discipline[c].mp < 7.0 &&
            student[a].discipline[c].mp >= 3.0)
        {
            printf("%s > %s > AF: ", student[a].name,
                                     student[a].discipline[c].name);
            scanf("%f", &nota);
